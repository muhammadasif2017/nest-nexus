...
describe('SessionGuard', () => {
  ...
  it('throws when the session user no longer exists', async () => {
    ...
    await expect(guard.canActivate(httpContext({ userId: 'u1' }))).rejects.toThrow(UnauthorizedException);
  });

  it('throws when the session user is deactivated', async () => {
    const prisma = makePrisma();
    prisma.user.findUnique.mockResolvedValue({ id: 'u1', email: 'a@b.com', roles: ['user'], isActive: false });
    const guard = new SessionGuard(prisma as any);
  ...
  it('serves from cache on the second call without hitting the database', async () => {
    const prisma = makePrisma();
    prisma.user.findUnique.mockResolvedValue({ id: 'u1', email: 'a@b.com', roles: ['user'], isActive: true });
    const guard = new SessionGuard(prisma as any);
